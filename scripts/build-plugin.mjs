...
//  - 源码：plugins-dev/<id>/src/entry.ts|.mts|.js（npm 依赖随意用）
...
// 入口优先 entry.ts（TS 插件），回退 .mts/.js（兼容遗留 JS 单文件版）
const entryCandidates = ['entry.ts', 'entry.mts', 'entry.js'].map((f) => join(srcDir, 'src', f))
...
if (!entry || !existsSync(manifest)) {
  console.error(`[build-plugin] 缺少 ${join(srcDir, 'src', 'entry.{ts,mts,js}')} 或 ${manifest}`)
  process.exit(1)
}
